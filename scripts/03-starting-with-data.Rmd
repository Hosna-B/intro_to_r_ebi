...
CSV file that contains the gene expression data to our disk and subsequently use
...
folder named `"data"`. If you have followed the first script (*01-r-rstudio.Rmd*) 
this folder already exists.


```{r, eval = TRUE}
# you may have to copy this command and manually paste it into your console to download the data.
...
```{r, eval = TRUE}
# notice how we use a relative path, because location of the R project 
# already sets a default working directory
...
tab is used as a field delineator. If you want to read in
this type of files in R, you can use the `read.delim()` function.
It is important to note that all of these functions are actually wrapper 
functions for the main `read.table()` function with different arguments.  
As such, the data above could have also been loaded by using `read.table()`
...
rna[1, 6]
# if we want to extract all elements of a certain column, we simply
# do not specify any row number:
# this gets first column of the data frame (as a vector)
rna[, 1]
...
columns. The values are sorted along the columns, as illustrated
below.
...
Hint: Check `?rnorm` to see how to generate random normally distributed numbers 
...
str(l)
```
Using `[]` you can subset a new sub-list. `[[]]` allows you to extract a single 
element of that list (using indices or names, of the list is named).
```{r}
l[[1]] # first element
...
```{r, eval = FALSE}
# you may have to copy this command and manually paste it into your console to save the data.
