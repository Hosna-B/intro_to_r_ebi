...
- It is recommended to save the object in an RDS file instead of saving them as 
RData. When loading an RDS file, you can set the name of the object you are 
loading. But with objects stored in an RData, this can't be done. This might rewrite
some objects in the environment if they have the same name as the object that is 
being loaded. 

- Regarding file naming, spaces are allowed but not recommended

- Difference between _package_ and _library_ in R

- Version control source

- R markdown vs R and knitting  
